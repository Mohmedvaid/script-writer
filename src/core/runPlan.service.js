...
function modelCharCapacity(tokenLimit, charsPerTok = 4) {
  return Math.floor(tokenLimit * charsPerTok * 0.9); // 10 % safety margin
}
...
function buildPlan({ title, pov, channel, styleKey }) {
  ...
  const charCap = modelCharCapacity(cfg.SCRIPT_MAX_TOKENS);
  const target  = reg.targetScriptChars;

  const segmentChars =
    target <= charCap
      ? target                    // one shot
      : Math.ceil(target / Math.ceil(target / charCap)); // evenly split
